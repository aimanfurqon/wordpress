...
function mudita_save_sidebar_layout( $post_id ) { 
    ...
    $layout = isset( $_POST['mudita_sidebar_layout'] ) ? sanitize_key( $_POST['mudita_sidebar_layout'] ) : 'right-sidebar';

    if( array_key_exists( $layout, $mudita_sidebar_layout ) ){
        update_post_meta( $post_id, 'mudita_sidebar_layout', $layout );
    }else{
        delete_post_meta( $post_id, 'mudita_sidebar_layout' );
    }
}
